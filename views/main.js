    ...
    <body>
      ...
      <div class="helvetica wrapper center">
        <div class="flex center pv3 w-80 w-40-l w-50-ns w-80-m">
          ...
          <button onclick=${e => emit('player:join', document.getElementById('player').value)} class="br4 pointer outline-0 W-40 w-25-l w-25-m w-25-ns bl-0 br--right pa3 dim b--gray ba bg-washed-yellow hover-bg-yellow fw7 dark-gray tracked-mega">Play</button>
        </div>
      </div>
      <div class="helvetica w-100">
        <table class="collapse ba b--black-10 pv2 center w-80 w-25-l w-33-ns w-50-m">
          ...
          <tbody>
          ${state.players ? state.players.slice(0, 5).map((user, i) => {
            return html`
